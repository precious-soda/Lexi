...
class CFG:
    DATASET_DIR = 'handwritten-kannada-characters-main'
    ...
    METRICS_LOG_FILE = os.path.join(OUTPUT_DIR, 'metrics_log.txt')
...
class KannadaDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            print(f"Error loading or processing image {image_path}: {e}. Returning placeholder.")
...
def train_one_epoch(model, loader, optimizer, scheduler, loss_fn, scaler, device, epoch, writer, train_metrics, log_file):
    model.train()
    total_loss = 0.0
    ...
    with open(log_file, 'a') as f:
        f.write(f"Epoch {epoch+1} Train Summary: Avg Loss: {avg_loss:.4f}, Accuracy: {accuracy:.2f}%\n")
...
def validate_one_epoch(model, loader, loss_fn, metrics_calculator, device, epoch, writer, val_metrics, log_file):
    model.eval()
    total_loss = 0.0
    num_batches = len(loader)
    ...
    with open(log_file, 'a') as f:
        f.write(f"Epoch {epoch+1} Val Summary: Avg Loss: {avg_loss:.4f}, Accuracy: {accuracy:.2f}%\n")
...
@torch.no_grad()
def evaluate_model(model, loader, loss_fn, metrics_calculator, device, class_names, log_file):
    model.eval()
    total_loss = 0.0
    num_batches = len(loader)
    metrics_calculator.reset()
    ...
    with open(log_file, 'a') as f:
        f.write("Running final evaluation on test set...\n")
    ...
    for name, value in final_metrics.items():
        ...
        with open(log_file, 'a') as f:
            f.write(f"{name.replace('test_', '')}: {value.item():.4f}\n")
...
def plot_training_curves(train_losses, train_accs, val_losses, val_accs, output_dir):
    ...
    ax2.plot(epochs, train_accs, label='Train Accuracy', color='blue', linestyle='-')
    ax2.plot(epochs, val_accs, label='Val Accuracy', color='orange', linestyle='--')
...
if __name__ == '__main__':
    ...
    with open(CFG.METRICS_LOG_FILE, 'w') as f:
        f.write(f"Configuration:\n{ {k: v for k, v in CFG.__dict__.items() if not k.startswith('__')} }\n")
        f.write(f"Using device: {CFG.DEVICE}\n")
    print(f"Using device: {CFG.DEVICE}")
    
    print("\n--- Loading Data ---")
    with open(CFG.METRICS_LOG_FILE, 'a') as f:
        f.write("\n--- Loading Data ---\n")
    ...
    with open(CFG.METRICS_LOG_FILE, 'a') as f:
        f.write(f"Data split: Train={len(train_samples)}, Val={len(val_samples)}, Test={len(test_samples)}\n")
    ...
    val_metrics_dict = {'loss': [], 'accuracy': []}
    ...
    for epoch in range(CFG.EPOCHS):
        train_loss, train_acc = train_one_epoch(model, train_loader, optimizer, scheduler, loss_fn, scaler, CFG.DEVICE, epoch, writer, train_metrics, CFG.METRICS_LOG_FILE)
        val_loss, val_acc = validate_one_epoch(model, val_loader, loss_fn, val_metrics, CFG.DEVICE, epoch, writer, val_metrics_dict, CFG.METRICS_LOG_FILE)
        
        plot_training_curves(train_metrics['loss'], train_metrics['accuracy'], val_metrics_dict['loss'], val_metrics_dict['accuracy'], CFG.VISUALIZATION_DIR)
        ...
        if val_acc > best_val_accuracy:
            ...
            with open(CFG.METRICS_LOG_FILE, 'a') as f:
                f.write(f"Epoch {epoch+1}: Validation accuracy improved to {val_acc:.2f}%. Checkpoint saved to {checkpoint_path}\n")
            epochs_no_improve = 0
        else:
            epochs_no_improve += 1
        ...
        if epochs_no_improve >= patience:
            ...
            with open(CFG.METRICS_LOG_FILE, 'a') as f:
                f.write(f"Early stopping triggered after epoch {epoch+1}\n")
            break
    ...
    with open(CFG.METRICS_LOG_FILE, 'a') as f:
        f.write(f"\n--- Training Complete ---\n")
        f.write(f"Best Validation Accuracy: {best_val_accuracy:.2f}% at Epoch {best_epoch+1}\n")
    ...
    if os.path.exists(best_checkpoint_path):
        ...
        with open(CFG.METRICS_LOG_FILE, 'a') as f:
            f.write(f"Loaded best model checkpoint from {best_checkpoint_path}\n")
        ...
        if loaded_mean != train_mean or loaded_std != train_std:
            ...
            with open(CFG.METRICS_LOG_FILE, 'a') as f:
                f.write("Warning: Normalization stats from checkpoint differ from current run.\n")
            
        evaluate_model(model, test_loader, loss_fn, test_metrics, CFG.DEVICE, class_names, CFG.METRICS_LOG_FILE)
    else:
        print("Warning: Best checkpoint not found. Evaluating the model from the last epoch.")
        with open(CFG.METRICS_LOG_FILE, 'a') as f:
            f.write("Warning: Best checkpoint not found. Evaluating the model from the last epoch.\n")
        evaluate_model(model, test_loader, loss_fn, test_metrics, CFG.DEVICE, class_names, CFG.METRICS_LOG_FILE)
