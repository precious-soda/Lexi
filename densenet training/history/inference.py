...
class CFG:
    ...
    PRETRAINED = True  # Updated to match training code
    DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    
    MODEL_CHECKPOINT_DIR = os.path.join('output', 'checkpoints', 'kannada_densenet121')
    MODEL_CHECKPOINT_FILE = os.path.join(MODEL_CHECKPOINT_DIR, 'kannada_densenet121_best.pth')
    LABEL_CSV_PATH = os.path.join('handwritten-kannada-characters', 'label.csv')
...
def build_model(cfg):
    if cfg.MODEL_ARCHITECTURE == 'densenet121':
        model = models.densenet121(weights='DenseNet121_Weights.DEFAULT' if cfg.PRETRAINED else None)
        
        original_conv0 = model.features.conv0
        model.features.conv0 = nn.Conv2d(1, original_conv0.out_channels,
        ...
        model.classifier = nn.Sequential(
            nn.Dropout(p=0.5),  # Added to match training code
