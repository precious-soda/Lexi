...
class CFG:
    ...
    OUTPUT_DIR = 'output'
    MODEL_NAME = 'kannada_densenet121' 
    ...
    N_MISCLASSIFIED_EXAMPLES = 25 
    N_AUGMENTATION_EXAMPLES = 16 
...
def get_data_paths_and_labels(characters_dir):
    ...
    if found_indices != expected_indices:
        ...
        if missing:
            print(f"Error: Expected class indices missing: {sorted(list(missing))}")
        if extra:
             print(f"Error: Unexpected class indices found: {sorted(list(extra))}")
...
def build_model(cfg):
    print(f"Building model: {cfg.MODEL_ARCHITECTURE}")
...
if __name__ == '__main__':
    ...
    if CFG.SCHEDULER == 'OneCycleLR':
        scheduler = optim.lr_scheduler.OneCycleLR(
            ...
            pct_start=0.2,  # Updated: 20% warmup
            anneal_strategy='cos',
