...
def load_model_and_stats(checkpoint_path, cfg):
    ...
    try:
        ...
        torch.serialization.add_safe_globals([getattr, OneCycleLR])  # Allowlist getattr and OneCycleLR
...
def save_predicted_text(segmentation_result, output_dir):
    try:
        output_file = os.path.join(output_dir, "predicted_text.txt")
...
def process_image(image_path, main_model, ott_model, main_mean, main_std, ott_mean, ott_std, main_class_names, ott_class_names, output_dir):
    ...
    save_predicted_text(segmentation_result, output_dir)
