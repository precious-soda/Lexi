...
def preprocess_for_prediction(roi, is_segmented=False):
    try:
        if not is_segmented:
            # Original preprocessing for non-segmented images
...
def process_image(image_path, model, train_mean, train_std, class_names, output_dir):
...
def compute_metrics(segmentation_result):
                ...
                for ott in char["ottaksharas"]:
                    ...
                    if ott_pred:
                        predictions_summary.append({
                            ...
                            "word": wi,
                            "char": total_chars - 1,
                            "ottakshara": total_ottaksharas - 1,
    ...
    for pred in predictions_summary:
        if pred["type"] == "main":
            print(f"Line {pred['line']}, Word {pred['word']}, Char {pred['char']}: "
                  f"Predicted '{pred['class_name']}' (Confidence: {pred['confidence']:.4f})")
        else:
            print(f"Line {pred['line']}, Word {pred['word']}, Char {pred['char']}, Ottakshara {pred['ottakshara']}: "
                  f"Predicted '{pred['class_name']}' (Confidence: {pred['confidence']:.4f})")
...
if __name__ == "__main__":
    ...
    try:
        ...
        seg_result = process_image(image_path, model, train_mean, train_std, class_names, output_dir)
