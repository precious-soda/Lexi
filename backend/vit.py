...
def build_model(cfg):
    model = timm.create_model('vit_base_patch16_224', pretrained=True, num_classes=cfg.NUM_CLASSES, img_size=cfg.IMAGE_SIZE, in_chans=3)
    model.head = nn.Sequential(
        nn.Dropout(p=0.5),
        nn.Linear(model.head.in_features, cfg.NUM_CLASSES)
    )
    nn.init.xavier_uniform_(model.head[1].weight)
...
def preprocess_for_prediction(roi, is_segmented=False):
    try:
        ...
        sharp_char_canvas = np.repeat(sharp_char_canvas[:, :, np.newaxis], 3, axis=2)
...
def predict_image(model, roi, train_mean, train_std, class_names, device, is_segmented=False):
    ...
    transform = T.Compose([
        ...
        T.Normalize(mean=[train_mean] * 3, std=[train_std] * 3)
    ])
